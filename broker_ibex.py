...
for i, f in enumerate(favoritos):
    try:
        ...
        if not d.empty:
            ...
            p_actual = float(d['Close'].iloc[-1].iloc[0] if isinstance(d['Close'].iloc[-1], pd.Series) else d['Close'].iloc[-1])
...
# --- ANALIZADOR CON GRÁFICA CORREGIDA ---
...
if ticker:
    try:
        ...
        if not df.empty:
            # LIMPIEZA DE DATOS (El truco para que no falle)
            df_plot = df['Close'].copy()
            if isinstance(df_plot, pd.DataFrame):
                df_plot = df_plot.iloc[:, 0] # Nos quedamos con la primera columna si hay lío
            
            df_final = pd.DataFrame({'Fecha': df_plot.index, 'Precio': df_plot.values})
            ...
            with c2:
                # GRÁFICA CON ZOOM AUTOMÁTICO
                fig = px.line(df_final, x='Fecha', y='Precio', color_discrete_sequence=['#FF4B4B'])
                fig.update_yaxes(autorange=True, fixedrange=False)
                fig.update_layout(
                    paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
                    font_color="white", margin=dict(l=0, r=0, t=20, b=0), height=350
                )
                st.plotly_chart(fig, use_container_width=True)
    except Exception as e:
        st.error("Yahoo está enviando datos, espera un segundo...")
